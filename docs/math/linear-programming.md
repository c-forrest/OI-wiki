...
线性规划（linear programming, LP）是研究线性约束条件下线性目标函数极值问题的方法总称，是运筹学的一个分支，在多方面均有应用。线性规划的某些特殊情况，如网络流、多商品流量等问题都有可能在算法竞赛题目中出现。
...
这就是一个典型的线性规划问题。它的目标函数和所有约束条件都是关于决策变量线性的。
...
当问题涉及多于两个决策变量时，图解法不再适用。但是，本节的例子中的一些观察仍然有效。线性规划问题中的每个不等式约束都描述了一个「半平面」，所有可行的解的集合就是这些「半平面」的交集，因此，总是一个「凸多边形」。规划问题的最优解总是可以在该「凸多边形」的某个「顶点」处取得。将这些观察拓展到高维空间，就发展出了一个高效的求解线性规划问题的方法——单纯形法。这也是算法竞赛中最常应用的方法。
...
也就是说，线性规划问题是最小化问题，所有决策变量都有非负约束，除此之外只包含若干右侧常量非负的等式约束。利用 [矩阵](./linear-algebra/matrix.md) 可以更为简洁地表达这一问题：
...
标准形式的选取只是为了行文方便，而并没有任何特别之处，因为任何线性规划问题都可以等价地写成下面的四种形式：
...
通过这些操作转化得到的线性规划问题的规模不超过原问题的规模的二倍，而且这些问题的可行解和最优解都很容易相互转化。因此，对于一般形式的线性规划问题，总是可以首先将它转化为标准形式（或上述四种形式之一）再进行求解。
...
所有可行解的集合 $\mathcal D\subseteq\mathbf R^n$ 称为线性规划问题 $P$ 的 **可行域**（feasible region）。从几何角度看，每个不等式约束 $a_j^T x \le b_j$ 都描述了一个半空间 $\{x\in\mathbf R^n:a_j^T x \le b_j\}$，每个等式约束 $a^T_jx = b_j$ 都描述了一个超平面 $\{x\in\mathbf R^n:a_j^Tx=b_j\}$，所以，可行域一定是有限多个半空间和超平面的交集。在优化领域[^poly-names]，它们通常称为 $\mathbf R^n$ 中的 **多面体**（polyhedron）。多面体一定是闭凸集，但未必是有界的。有界的多面体也称为 **多胞形**（polytope）。多胞形可以看作是平面上的多边形在高维空间的推广，而多面体将它进一步推广到可能无界的情形。

理解多面体的结构有助于理解线性规划的解。对于多面体 $\mathcal D\in\mathbf R^n$ 和向量 $c\in\mathbf R^n\setminus\{0\}$，考虑如下的线性规划问题 $P$：

$$
\max\{c^Tx:x\in\mathcal D\}.
$$

从几何角度来看，这相当于在超平面 $H : c^T x = z$ 与可行域 $\mathcal{D}$ 至少有一个交点的前提下，沿着向量 $c$ 的方向移动超平面 $H$，使得 $z$ 尽可能大。这就存在三种可能性：

-   可行域 $\mathcal D$ 是空集。这说明问题 $P$ 没有可行解，它的某些约束是相互矛盾的。此时，称问题 $P$ 是 **不可行的**（infeasible），它的最优价值规定为 $-\infty$。

-   可行域 $\mathcal D$ 非空，但是它包含一条方向向量为 $c$ 的射线。因为沿着向量 $c$ 的方向可以不断地移动超平面 $H$，且至少集合 $H\cap \mathcal D$ 含有这条射线里的某个点，它就一定是非空的，所以，目标函数 $c^Tx$ 可以取得任意大的值。此时，称问题 $P$ 是 **无界的**（unbounded），它的最优价值规定为 $+\infty$。

-   可行域 $\mathcal D$ 非空，且不含有任何方向向量为 $c$ 的射线。此时，问题 $P$ 存在有限的最优价值，记为 $z^*$。超平面 $H^*:c^Tx = z^*$ 与可行域 $\mathcal D$ 相交，且 $\mathcal D$ 包含于半空间 $\{x:c^Tx\le z^*\}$ 中，此时，超平面 $H^*$ 称为 $\mathcal D$ 的一个 **支撑超平面**（supporting hyperlane）。显然，问题 $P$ 的最优解集就是 $H^*\cap\mathcal D$。它一定是多面体，且包含在 $\mathcal D$ 的边界中。它称为多面体 $\mathcal D$ 的一个 **面**（face）。形象地说，多面体正是由这些面围成的。除此之外，$\mathcal D$ 本身也看作是 $\mathcal D$ 的一个面。

    一个 $d$ 维的多面体的面的维度可能是 $0$ 到 $d$ 之间的任何整数。维度为 $0$ 的面（即一个点）称为多面体 $\mathcal D$ 的 **顶点**（vertex）或 **角点**（corner point），维度为 $1$ 的面称为多面体 $\mathcal D$ 的 **边**（edge），维度为 $d-1$ 的面则称为多面体 $\mathcal D$ 的 **维面**（facet）。并非所有非空的多面体都有顶点，比如 $\{x\in\mathbf R^2:x_1\le 1\}$ 的最小维度的面就是一条直线，维度为 $1$。事实上，因为多面体 $\mathcal D$ 的面 $F$ 的面仍然是 $\mathcal D$ 的面，而只有仿射子空间（即若干个超平面的交集）才没有严格更小的非空面，所以，多面体 $\mathcal D$ 的所有极小面都是仿射子空间。而且，同一个多面体的极小面的维度是相同的。

    因为多面体的面给出了线性规划问题的解集，所以，一个自然的问题是，如何确定多面体的面的方程。设多面体 $\mathcal D$ 由若干个约束 $a_j^Tx \lesseqqgtr b_j$ 描述，且 $F$ 是 $\mathcal D$ 的一个面。如果某个约束在所有 $x\in F$ 处都取得等号，就称它在面 $F$ 上是 **紧的**（tight）。事实上，将所有紧约束取等号得到的方程联立，就确定了相应的面 $F$。因此，面 $F$ 的维度，就等于变量个数 $n$ 与所有这些紧约束的系数组成的矩阵的秩的差值。特别地，多面体的顶点可以通过联立 $n$ 个线性独立的紧约束得到。反过来说，任意选取约束的一个子集，将它们取作等号，联立得到的子空间和多面体 $\mathcal D$ 的交集就是后者的一个面。而且，选取的紧约束越多，就意味着得到的面（在集合包含意义下）越小。

这些讨论忽略了 $c=0$ 的情形。此时，显然线性规划问题不能是无界的，所以要么问题本身是不可行的，要么最优价值等于 $0$，且最优解集就是 $\mathcal D$ 本身——当然，它也是多面体 $\mathcal D$ 的一个面。这类特殊的线性规划也称为 **可行性线性规划**（feasibility linear programming）。

值得指出的是，判定线性规划问题是否可行、是否有界，以及求出不等式组的可行解等问题，都和解线性规划问题本身同样困难[^reducible]。比如说，下文中强对偶定理的证明就说明，解一个有界的线性规划问题，就相当于寻找另一组不等式的可行解。因此，判断不等式组是否有解，或者判断方程组是否有非负解等，最有效的方式就是求解相应的可行性线性规划。
...
算法竞赛中，很少有问题只能通过线性规划的算法解决。大多数可以用线性规划方法求解的题目，通常也可以通过网络流等更专门的算法来解决。
...
尽管单纯形法的最坏情况复杂度是指数级的，而内点法的复杂度是多项式的，但这两类算法在大多数实际问题中的表现都非常出色。相比之下，虽然椭球法的理论复杂度是多项式级别的，但是通常运行缓慢，并不实用。
...
其中，对偶问题的决策变量 $y_1,y_2,y_3$ 分别是原问题的三类约束的 Lagrange 乘子；反过来，原问题的决策变量 $x_1,x_2,x_3$ 也分别是对偶问题的三类约束的 Lagrange 乘子。显然，对偶问题的对偶问题就是原问题。
...
Farkas 实际上是一种 [超平面分离定理](https://en.wikipedia.org/wiki/Hyperplane_separation_theorem)。情形 1 是在说，点 $b$ 位于 $A$ 的列向量生成的凸锥 $C$ 里；因此，Farkas 引理说明，当且仅当点 $b$ 不在这一凸锥 $C$ 中时，存在经过原点且法向量为 $y$ 的超平面 $H:y^Tx = 0$ 强分离了点 $b$ 和凸锥 $C$ 。
...
??? note "证明"
    ...
    $$
    0 = (\mu^*)^T A^Ty_0 \le c^T\mu^* < b^T\lambda^* = x_0^TA^T\lambda^* \le 0.
    $$
...
??? note "证明"
    ...
    因此，互补松弛条件成立，当且仅当 $b^Ty^* = c^Tx^*$。根据强对偶定理，这一条件成立，当且仅当 $x^*$ 和 $y^*$ 分别是原问题的最优解。
...
整数约束显著增加了整数规划问题的复杂性。许多组合优化问题，例如背包问题、适定性问题以及众多图论中的优化问题，都可以表示为整数规划模型。而这些问题中的多数被证明是 NP 完全或 NP 困难的。
...
??? note "证明"
    ...
    记这个方程组为 $A_Jx=b_J$，且 $A_J=(A_1,A_2)$，其中，$A_1$ 是满秩的方阵。那么，解
...
常见的图论模型中，网络流、最短路、二分图等的线性规划问题的系数矩阵都是全幺模矩阵。因此，只需要这些问题仅涉及整数参数，它们的最优解就可以取作整数，而不用担心线性规划问题的解对应着分数流、分数匹配等情形。除此之外，还有一些常见的图论模型，可以通过巧妙的选取约束，使得可行域是顶点均为整点的多胞形。例如，一般图匹配和生成树等都存在顶点均为整点的多胞形。
