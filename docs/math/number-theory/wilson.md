...
本文中常见的想法是对于素数 $p$ 和正整数 $n$，要计算阶乘 $n!$ 在模 $p^\alpha$ 下的余数，可以将阶乘 $n!$ 中的所有因子 $p$ 都提取出来，得到分解：

$$
n! = p^{\nu_p(n!)}(n!)_p.
$$

其中，记号 $\nu_p(n!)$ 表示阶乘 $n!$ 的素因数分解中 $p$ 的幂次，记号 $(n!)_p$ 表示所有小于等于 $n$ 但不能被 $p$ 整除的正整数的乘积。本文将讨论 $(n!)_p$ 在素数（幂）模下的余数和幂次 $\nu_p(n!)$ 的具体计算方法。
...
???+ example "例子"
    ...
    可以看出，利用模 $5$ 余数的周期性，可以将这一乘积划分为若干个长度为 $5$ 的块，每一块的唯一差异就是最后一个元素的余数。因为 $32$ 除以 $5$ 得到的商是 $6$ 且余数是 $2$，所以，该乘积可以划分为 $6$ 个完整的块和最后一段长度为 $2$ 的不完整的块。因此，可以将前 $6$ 个块除了最后一个元素之外的部分提取出来（这一部分恰好是 Wilson 定理能够解决的），再乘上最后一个不完整的块的乘积，最后乘上前 $6$ 个块的最后一个元素的连乘积。每个块的最后一个元素都是 $5$ 的倍数，它们的连乘积恰好是 $(6!)_{5}\pmod{5}$。这就将原来的问题转化为了规模更小的问题。
...
??? note "证明"
    ...
    最后一个部分块的值就是 $(n\bmod p)!\pmod p$，可以单独计算。
