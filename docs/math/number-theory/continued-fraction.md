...
有限连分数对应的是有理数。每个有理数都有且仅有两种方式可以表示成连分数。这两种方式唯一的区别在于最后一项是否为 $1$，即
...
??? example "例子"
    ...
    $$
    \begin{aligned}
    r = [1,1,1,1] &= 1+\dfrac{1}{1+\dfrac{1}{1+\dfrac{1}{1}}},\\
    r = [1,1,2] &= 1+\dfrac{1}{1+\dfrac{1}{2}}.
    \end{aligned}
    $$
...
此时的计算过程实际上是对 $p$ 和 $q$ 做 [辗转相除法](./gcd.md#欧几里得算法)。这也说明，对于有理数 $r=\dfrac{p}{q}$，连分数表示的长度大致是 $O(\log \min\{p, q\})$ 的。

??? example "参考实现"
    算法的输入是分数的分子和分母，输出的是连分数的系数序列。
...
-   直线 $y=\xi x$ 的方向向量是 $\vec\xi=(1,\xi)$。利用 [叉积](../linear-algebra/product/#二维向量的情形) $(x_1,y_1)\times(x_2,y_2)=x_1y_2-x_2y_1$ 的概念，可以通过 $\vec\xi\times\vec\nu=p-q\xi$ 的正负判断点在直线上方还是下方。因而，在直线上方的点就对应着大于等于 $\xi$ 的分数，在直线下方的点就对应着小于等于 $\xi$ 的分数。叉积的绝对值 $|\vec\xi\times\vec\nu|$ 正比于点 $\vec\nu$ 与直线 $y=\xi x$ 的距离
...
Stern–Brocot 树是存储了所有位于 $[0,\infty]$ 之间的分数的 [二叉搜索树](../../ds/bst)。有限连分数实际上编码了 Stern–Brocot 树上从根到某个分数所在位置的路径。也就是说，有理数 $x$ 的连分数表示 $[a_0,a_1,\cdots,a_{n-1},1]$ 意味着从树根 $\dfrac{1}{1}$ 开始，需要先向右子节点移动 $a_0$ 次，再向左子节点移动 $a_1$ 次，交替方向移动，直到向某个方向移动了 $a_{n-1}$ 次为止。应当注意，此处只能使用末尾为 $1$ 的连分数表示。
...
Lagrange 的结果[^lagrange-theorem]说明反过来也成立，因而二次无理数和循环连分数是等价的。
...
定理的证明中，显式地提供了一个计算二次无理数的余项的递推公式：
...
??? "解答"
    首先将二次无理数表示成上述形式，在利用递推公式计算即可。为了求出循环节，需要存储 $(P_k,Q_k)$ 的值。
