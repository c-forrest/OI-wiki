...
组合优化问题中，证明价值函数的凸性常常会用到交换论证（exchange argument）。具体地说，就是从参数为 $k-1$ 和 $k+1$ 的最优解出发，通过交换部分元素，构造出参数为 $k$ 且价值函数不超过 $(v(k-1)+v(k+1))/2$ 的可行解，从而证明凸性的论证方法。相较于凸优化的情形，组合优化问题中并不存在自然地构造两个解的「中间形态」的方法，因此，该方法的应用通常具有一定的技巧性。
...
???+ warning "「边际成本递增」并不一定导致凸性"
    ...
    **未必** 是 $k$ 的凸函数。究其原因，就是因为从子集大小分别为 $k-1$ 和 $k+1$ 的最优解，一般来说是无法构造出子集大小为 $k$ 且满足前述价值函数大小关系的可行解。
